...
enum Commands {
    //Lista los  grupos APT
    #[command(name = "apt-list")]
    Aptlist,
    //Muestra info de grupo APT
    #[command(name = "apt")]
    Apt { name: String},
    // Muestra info de la técnica basado en ID (e., T1055)
    #[command(name = "tid")]
    TecId { id: String},
...
fn get_matrix_path() -> PathBuf {
    let home = dirs::home_dir().expect("No se encontró la carpeta home");
...
fn load_attack_data() -> Result<AttackData, Box<dyn std::error::Error>> {
    ...
    if !path.exists(){
        eprintln!("{}", "Error: Matriz MITRE ATT&CK no encontrada en ~/.mitre/matrix.json".red());
        eprintln!("{}","Por favor ejecute primero el cript de instalación".yellow());
        std::process::exit(1);
    }
...
fn get_related_techniques<'a>(group_id: &str, data: &'a AttackData) -> Vec<&'a AttackObject> {
        ...
        if obj.obj_type == "relationship" {
            if let (Some(source_ref), Some(target_ref), Some(relationship_type)) =
                ...
                if source_ref == group_id && relationship_type == "uses" {
                    related_techniques_ids.push(target_ref.as_str());
                }
            }
        }
    }
    ...
    for obj in &data.objects {
        if obj.obj_type == "attack-pattern" && related_techniques_ids.contains(&obj.id.as_str()) {
            techniques.push(obj);
        }
    }
...
fn get_retaled_groups<'a>(technique_id: &str, data: &'a AttackData) -> Vec<&'a AttackObject> {
    let mut related_group_ids = Vec::new();

    // Busca todas las relaciones donde los grupos utilices esta técnica
    ...
    // Obtener el objeto actual del grupo
...
// Formateo de impresiones en pantalla
...
fn print_technique_info(obj: &AttackObject, data: &AttackData) {
    println!("{}", format!("Nombre: {}", obj.name.as_ref().unwrap_or(&"Desconocido".to_string())).bright_cyan().bold());
    ...
    let related_groups = get_retaled_groups(&obj.id, data);
    if !related_groups.is_empty() {
        println!("\n{}", "Usado por los grupos:".bright_white().bold());
...
fn print_group_info(obj: &AttackObject, data: &AttackData) {
    println!("{}", format!("Nombre: {}", obj.name.as_ref().unwrap_or(&"Desconocido".to_string())).on_bright_cyan().bold());
    ...
    println!("{}", format!("Tipo: {}", obj.obj_type).bright_yellow());
    ...
    // Buscar técnicas relacionadas a través de la familiaridad

    let related_techniques = get_related_techniques(&obj.id, data);
    if !related_techniques.is_empty() {
        println!("\n{}", "Técnicas usadas:".bright_white().bold());

        // Técnicas del grupo por tácticas
        ...
        // Ordenar tácticas alfabéticamente
        ...
        println!("\n{}", format!("Total Técnicas: {}", related_techniques.len()).bright_cyan());
...
fn print_tactic_info(obj: &AttackObject) {
    println!("{}", format!("Name: {}", obj.name.as_ref().unwrap_or(&"Desconocido".to_string())).bright_cyan().bold());

    if let Some(shortname) = &obj.shortname {
        println!("{}", format!("Abreviatura: {}", shortname).bright_green());
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match &cli.command {
        Commands::Aptlist => {
            println!("{}", "Grupos APT (Advanced Persistent Threat):".bright_cyan().bold());
            print_separator();
        ...
        for group in groups {
            if let Some(name) = &group.name {
                ...
                if let Some(aliases) = &group.aliases {
                    ...
                    println!(" Alias: {}", alias_str.bright_black());
                }
                println!();
            }
        }
    ...
    Commands::Apt { name } => {
        ...
        for obj in &data.objects {
            if obj.obj_type == "intrusion-set" {
                ...
                // Verificación de nombre del grupo
        ...
        if found_groups.is_empty() {
                println!("{}", format!("Grupo APT no encontrado '{}'", name).red());
        } else {
    ...
    Commands::TecId { id } => {
        ...
        if !found {
            println!("{}", format!("Técnica con ID '{}' no encontrada", id).red());

        }
    },

    Commands::TecName { name } => {
        let name_lower = name.to_lowercase();
        let mut found = false;
    ...
    Commands::Tac { name } => {
        ...
        if found {
            println!("\n{}", "Técnicas Relacionadas:".bright_white().bold());
            print_separator();
